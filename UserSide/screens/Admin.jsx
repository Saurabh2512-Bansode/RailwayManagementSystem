...
const Admin = () => {
  ...
  const addOrUpdateAdmin = async () => {
    try {
      if (editingAdmin) {
        await axios.put(`http://localhost:9999/admin/${editingAdmin.AdminId}`, adminData);
      } else {
        await axios.post("http://localhost:9999/admin", adminData);
      }
      fetchAdmins();
  ...
  const deleteAdmin = async (AdminId) => {
    try {
      await axios.delete(`http://localhost:9999/admin/${AdminId}`);
      fetchAdmins();
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <Modal visible={modalVisible} animationType="slide">
        <View style={styles.modalContainer}>
          <Text style={styles.sectionTitle}>{editingAdmin ? "Edit Admin" : "Add Admin"}</Text>
          ...
          <TouchableOpacity style={styles.addButton} onPress={addOrUpdateAdmin}>
            <Text style={styles.addButtonText}>{editingAdmin ? "Update Admin" : "Save Admin"}</Text>
          </TouchableOpacity>
