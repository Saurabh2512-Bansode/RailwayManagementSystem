...
const Train = () => {
  ...
  const fetchTrains = async () => {
    try {
      const response = await axios.get("http://localhost:9999/train");
  ...
  const addOrUpdateTrain = async () => {
    try {
      if (editingTrain) {
        await axios.put(`http://localhost:9999/train/${editingTrain.TrainNo}`, trainData);
      } else {
        await axios.post("http://localhost:9999/train", trainData);
      }
      fetchTrains();
      resetForm();
    } catch (error) {
  ...
  const deleteTrain = async (TrainNo) => {
    try {
      await axios.delete(`http://localhost:9999/train/${TrainNo}`);
      fetchTrains();
    } catch (error) {
