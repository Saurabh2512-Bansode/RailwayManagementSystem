...
const Station = () => {
  ...
  const fetchStations = async () => {
    try {
      const response = await axios.get("http://localhost:9999/station");
  ...
  const addOrUpdateStation = async () => {
    try {
      if (editingStation) {
        await axios.put(`http://localhost:9999/station/${editingStation.StationNo}`, stationData);
      } else {
        await axios.post("http://localhost:9999/station", stationData);
      }
      fetchStations();
  ...
  const deleteStation = async (StationNo) => {
    try {
      await axios.delete(`http://localhost:9999/station/${StationNo}`);
      fetchStations();
    } catch (error) {
