...
const Route = () => {
  ...
  const fetchRoutes = async () => {
    try {
      const response = await axios.get("http://localhost:9999/route");
  ...
  const addOrUpdateRoute = async () => {
    try {
      if (editingRoute) {
        await axios.put(`http://localhost:9999/route/${editingRoute.RouteId}`, routeData);
      } else {
        await axios.post("http://localhost:9999/route", routeData);
      }
      fetchRoutes();
  ...
  const deleteRoute = async (RouteId) => {
    try {
      await axios.delete(`http://localhost:9999/route/${RouteId}`);
      fetchRoutes();
    } catch (error) {
