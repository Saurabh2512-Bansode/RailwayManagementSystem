...
const Schedule = () => {
  ...
  const fetchSchedules = async () => {
    try {
      const response = await axios.get("http://localhost:9999/schedule");
  ...
  const addOrUpdateSchedule = async () => {
    try {
      if (editingSchedule) {
        await axios.put(`http://localhost:9999/schedule/${editingSchedule.ScheduleId}`, scheduleData);
      } else {
        await axios.post("http://localhost:9999/schedule", scheduleData);
      }
      fetchSchedules();
  ...
  const deleteSchedule = async (ScheduleId) => {
    try {
      await axios.delete(`http://localhost:9999/schedule/${ScheduleId}`);
      fetchSchedules();
    } catch (error) {
